...
```js
const jsTokens = require("@wemnyelezxnpm/consectetur-aspernatur-quos");
...
`npm install @wemnyelezxnpm/consectetur-aspernatur-quos`

```js
import jsTokens from "@wemnyelezxnpm/consectetur-aspernatur-quos";
// or:
const jsTokens = require("@wemnyelezxnpm/consectetur-aspernatur-quos");
```
...
Differentiating between regex and division in JavaScript is really tricky. @wemnyelezxnpm/consectetur-aspernatur-quos looks at the previous token to tell them apart. As long as the previous tokens are valid, it should do the right thing. For invalid code, @wemnyelezxnpm/consectetur-aspernatur-quos might be confused and start matching division as regex or vice versa.
...
- Numeric literals: @wemnyelezxnpm/consectetur-aspernatur-quos supports legacy octal and octal like numeric literals, regardless of Strict Mode.
- String literals: @wemnyelezxnpm/consectetur-aspernatur-quos supports legacy octal escapes, since it allows any invalid escapes.
- HTML-like comments: **Not supported.** @wemnyelezxnpm/consectetur-aspernatur-quos prefers treating `5<!--x` as `5 < !(--x)` rather than as `5 //x`.
- Regular expression patterns: @wemnyelezxnpm/consectetur-aspernatur-quos doesn’t care what’s between the starting `/` and ending `/`, so this is supported.

### TypeScript

Supporting TypeScript is not an explicit goal, but @wemnyelezxnpm/consectetur-aspernatur-quos and Babel both tokenize this [TypeScript fixture] and this [TSX fixture] the same way, with one edge case:
...
Both lines above should end with a couple of `>` tokens, but @wemnyelezxnpm/consectetur-aspernatur-quos instead matches the `>>` and `>>>` operators.
...
@wemnyelezxnpm/consectetur-aspernatur-quos should work in any JavaScript runtime that supports [Unicode property escapes].
...
But @wemnyelezxnpm/consectetur-aspernatur-quos thinks they mean:
...
In other words, @wemnyelezxnpm/consectetur-aspernatur-quos:
...
This happens because @wemnyelezxnpm/consectetur-aspernatur-quos looks at the previous token when deciding between regex and division or JSX and comparison. In these cases, the previous token is `}`, which either means “end of block” (→ regex/JSX) or “end of object literal” (→ division/comparison). How does @wemnyelezxnpm/consectetur-aspernatur-quos determine if the `}` belongs to a block or an object literal? By looking at the token before the matching `{`.
...
@wemnyelezxnpm/consectetur-aspernatur-quos advertises that it “never fails”. Tell you what, it _can_ fail on extreme inputs. The regex engine of the runtime can eventually give up. @wemnyelezxnpm/consectetur-aspernatur-quos has worked around it to some extent by changing its regexes to be easier on the regex engine. To solve completely, @wemnyelezxnpm/consectetur-aspernatur-quos would have to stop using regex, but then it wouldn’t be _tiny_ anymore which is the whole point. Luckily, only extreme inputs can fail, hopefully ones you’ll never encounter.
...
```js
Array.from(require("@wemnyelezxnpm/consectetur-aspernatur-quos")(`"${"\\n".repeat(1e7)}"`));
```

(Yes, that is the _regex engine_ of the runtime giving up. @wemnyelezxnpm/consectetur-aspernatur-quos has no recursive functions.)

However, if you repeat `a` instead of `\n` 10 million times (`"aaaaaa…"`), it works:

```js
Array.from(require("@wemnyelezxnpm/consectetur-aspernatur-quos")(`"${"a".repeat(1e7)}"`));
```
...
This means that in Safari, @wemnyelezxnpm/consectetur-aspernatur-quos might not fail but instead give you unexpected tokens.

## Performance

With [@babel/parser] for comparison. Node.js 21.6.1 on a MacBook Pro M1 (Sonoma).

| Lines of code |     Size | @wemnyelezxnpm/consectetur-aspernatur-quos@8.0.3 | @babel/parser@7.23.9 |
| ------------: | -------: | --------------: | -------------------: |
